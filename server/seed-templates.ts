...
TONO: cálido, como darle la bienvenida a alguien que tomó una buena decisión. Profesional pero cercano.`,
  ...
  {
    nombre: "mini_auditoria",
    subjectPrompt: "Genera un subject que transmita que analizaste el diagnóstico y encontraste oportunidades. Máximo 60 caracteres.",
    bodyPrompt: "Mini-auditoría generada por generateMiniAudit() — este template es placeholder para referencia en la secuencia.",
    sequenceOrder: 0,
    delayDays: 0,
  },
  {
...
- Estructura: saludo con nombre → dato impactante de industria → caso de éxito → resultado → puente a la cita (al final)

PROHIBIDO: NO abras con referencias a la confirmación de la cita, "vemos que agendaste", "como ya tienes tu sesión programada", ni nada similar. La persona ya lo sabe — este es el 2do email. Ve DIRECTO al contenido de valor.

CONTENIDO OBLIGATORIO:
1. "{participante}," — saludo con nombre, nunca genérico
2. INMEDIATAMENTE después del nombre: dato o tendencia impactante sobre {industria} con FUENTE VERIFICABLE (McKinsey, Gartner, Forrester, Deloitte, HBR). Ejemplo: "{participante}, el 67% de las empresas de {industria} ya están usando IA para..."
...
6. AL FINAL (no al inicio): puente breve a la cita: "Esto es justo lo que analizaremos en tu sesión del {fechaCita} a las {horaCita}"
...
INSTRUCCIONES ESPECÍFICAS:
- Largo: máximo 200 palabras
- Estructura: saludo con nombre → pregunta provocadora → 3 insights → diferenciador → puente a la cita (al final)

PROHIBIDO: NO abras con referencias a la confirmación de la cita, "como ya tienes agendada tu sesión", "sabemos que confirmaste", ni nada similar. Este es el 3er email — la persona ya sabe que tiene cita. Ve DIRECTO al contenido de valor.

CONTENIDO OBLIGATORIO:
1. "{participante}," — saludo con nombre
2. INMEDIATAMENTE después: pregunta provocadora o dato que enganche. Ejemplo: "¿Sabías que el 40% de los procesos de {industria} ya se pueden automatizar?" o "En {industria}, estas son las 3 áreas donde más impacto está teniendo la tecnología:"
...
5. AL FINAL: "Esto es justo lo que analizaremos juntos el {fechaCita} a las {horaCita}"
...
TONO: thought leadership. Como un consultor senior compartiendo lo que ve en el mercado. Que sienta que ya está aprendiendo cosas útiles.`,
  ...
  {
    nombre: "prep_agenda",
    subjectPrompt: "Genera un subject de preparación para la próxima sesión de diagnóstico. Máximo 50 caracteres. Ejemplo: 'Tu diagnóstico con IM3 se acerca, {participante}'.",
    bodyPrompt: `Genera un email de preparación y agenda para la próxima sesión de diagnóstico.

INSTRUCCIONES ESPECÍFICAS:
- Largo: máximo 180 palabras
- Estructura: dato relevante de industria → recordatorio → agenda → pregunta de preparación → botón reunión
- IMPORTANTE: La sesión es el {fechaCita} a las {horaCita}. Si es hoy, di "hoy". Si es mañana, di "mañana". Usa el contexto temporal correcto.

CONTENIDO OBLIGATORIO:
1. Abre con un dato o estadística relevante sobre cómo empresas de {industria} están usando IA o automatización (citar fuente: McKinsey, Gartner, Forrester, etc.). Conectar con sus {areaPrioridad}.
2. "{participante}, {fechaCita} a las {horaCita} veremos cómo aplicar esto específicamente en {empresa}"
3. Agenda de la sesión (bullets):
   - Auditoría de operaciones actuales y stack tecnológico
   - Mapeo de oportunidades de IA y automatización específicas para {industria}
   - Evaluación de viabilidad técnica basada en {herramientas}
   - Hoja de ruta sugerida con prioridades alineadas a {objetivos}
4. Pregunta de preparación: plantea una pregunta concreta que el contacto pueda pensar antes de la reunión, relacionada con {areaPrioridad}. Ejemplo: "Piensa en: ¿cuántas horas a la semana dedica tu equipo a [proceso relacionado]?"
5. Si hay meetLink: incluir como BOTÓN PRINCIPAL "Unirse a la reunión →"
6. Si hay link de agregar al calendario: "📅 Agregar a mi calendario"
7. Al final (texto pequeño, color #999): "Reagendar · Cancelar"

NO incluyas CTA a /booking. El único CTA prominente es el link de la reunión.

TONO: como un consultor que ya investigó sobre la empresa y su industria. Genera anticipación con datos concretos.`,
    sequenceOrder: 3,
    delayDays: 0,
  },
  {
...
- Si hay una SESIÓN DE SEGUIMIENTO YA AGENDADA en los datos del contexto:
  * NO incluyas CTA a /booking
  * En su lugar, confirma la sesión: "Te confirmo nuestra próxima sesión el [fecha] a las [hora]" (usa los datos reales del seguimiento)
  * Incluye el botón de Meet link de seguimiento prominente si está disponible
  * Tono: más confiado, como continuando una relación activa
- Si NO hay seguimiento agendado:
  * CTA: "¿Te gustaría agendar una sesión de seguimiento para revisar la propuesta?" con botón a https://www.im3systems.com/booking

IMPORTANTE: Este email se envía DESPUÉS de la reunión. Escribe en pasado ("en nuestra conversación de hoy", "como vimos juntos"). NO menciones la fecha/hora de la reunión original.
...
TONO: consultivo, con momentum. Como un consultor senior que quiere mantener el impulso.`,
  ...
  {
    nombre: "reengagement",
    subjectPrompt: "Genera un subject disruptivo que NO mencione auditoría, diagnóstico ni cita. Enfócate en un dato sorprendente de la industria del contacto. Máximo 60 caracteres.",
    bodyPrompt: "Re-engagement generado por generateReengagement() — este template es placeholder para referencia en la secuencia.",
    sequenceOrder: 98,
    delayDays: 0,
  },
  {
